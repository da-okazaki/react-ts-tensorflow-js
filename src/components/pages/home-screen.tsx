  ...
  return (
    <div className={classes.root}>
      ...
      <div className={classes.contentsTitle}>
        ...
        <Typography variant='subtitle2'>事前トレーニング済みの TensorFlow.js モデル</Typography>
      </div>
      <Grid container className={classes.gridContainerStyle}>
        {CardData.map((item) => (
          <Grid item xs={6} sm={4} className={classes.gridItemStyle} key={item.id}>
            <ImageCard image={item.image} title={item.title} label={item.label} path={item.path} />
          </Grid>
        ))}
      </Grid>
    </div>
